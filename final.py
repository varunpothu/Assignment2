...
def electricity_data(filename):
    ...
    years_data.columns = [col if not col.isdigit() else str(col) for col \
                          in years_data.columns]
...
def plot_electricity_use_by_country(data):  
    ...
    data_x = data.loc[data["Country Name"].isin(countries) & data["Indicator \
                     Code"].eq(indicator), ["Country Name"] + years]
...
def plot_gdp_per_energy_use(data): 
    ...
    data_y = data.loc[data["Country Name"].isin(countries) & data["Indicator \
                Code"].eq(indicator), ["Country Name"] + years]
...
def plot_electricity_production(data):
    ...
    data = data.loc[data["Country Name"].isin(countries) & data["Indicator \
        Code"].isin(indicators), ["Country Name", "Indicator Code"] + years]
